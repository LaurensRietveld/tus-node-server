...
class FileStore extends DataStore {
    constructor(options) {
        ...
        this.preUploadCompleteSignal = options.preUploadCompleteSignal;
    }
    ...
    writeMetadata(metadata) {
        const path = `${this.directory}/${metadata.id}.metadata`;
        return fs.promises.writeFile(path, JSON.stringify(metadata)).catch((e) => {
            log('[FileStore] write metadata: Failed creating metadata, ignoring', e);
        });
    }

    readMetadata(file_id) {
        const path = `${this.directory}/${file_id}.metadata`;
        return fs.promises.readFile(path, 'utf8').then((metadataString) => JSON.parse(metadataString)).catch((e) => {
            log('[FileStore] read metadata: Failed reading metadata, ignoring', e);
            return undefined;
        });
    }
    ...
    create(req) {
        return new Promise((resolve, reject) => {
            ...
            return fs.open(`${this.directory}/${file.id}`, 'w', (err, fd) => {
                ...
                return this.writeMetadata(file)
                  .then(() => {
    ...
    write(req, file_id, offset) {
        return new Promise((resolve, reject) => {
            ...
            req.on('data', (buffer) => {
                ...
                if(this.dataCallback) this.dataCallback(file_id, buffer);
            });
            ...
            return req.pipe(stream).on('finish', () => {
                ...
                this.readMetadata(file_id)
                  .then((config) => {
                      if (config && parseInt(config.upload_length, 10) === offset) {
                          var preUploadPromise = this.preUploadCompleteSignal ? this.preUploadCompleteSignal(config) : Promise.resolve();
                          preUploadPromise
                              .then((preUploadData) => {
                                  this.emit(EVENTS.EVENT_UPLOAD_COMPLETE, { file: config });
                                  resolve({offset:offset, payload: preUploadData} );
                              })
                              .catch((e)=>{
                                  if (e && e.status_code && e.body){
                                    return reject(e)
                                  }
                                  return reject(ERRORS.FILE_WRITE_ERROR);
                              });
     ...
     */
    getOffset(file_id) {
        return this.readMetadata(file_id)
            .then((config) => {
                return new Promise((resolve, reject) => {
